...
SCENARIO_LABELS = {
    "B_plus_D_then_order_by_OS": "B+D then order by next-day overall_score",
    "overall_score_only": "next-day overall_score only",
    "overall_score_plus_sleep": "next-day overall_score + same-date sleep",
    "baseline_A_B_D_no_OS_in_fit": "Baseline A+B+D (next-day OS for ordering only)",
}
...
def _extract_user_behavioral_profiles(user_payload: dict) -> dict:
    """Pull LLM behavioral-profile interpretations for every scenario (next-day + production)."""
    profiles: dict = {}
    for s in user_payload.get("scenarios") or []:
        ...
        production = s.get("llm_interpretation_production") or {}
        # Back-compat: older files may have used llm_interpretation_original
        if not production:
            production = s.get("llm_interpretation_original") or {}
        profiles[name] = {
            ...
            "current": {
                ...
                "prompt_variant": current.get("prompt_variant") or "next_day_os",
            ...
            "original": {
                "profile_version": production.get("profile_version"),
                "prompt_variant": production.get("prompt_variant")
                or "production_same_day_wording",
                "summary": production.get("summary") or "",
                "primary_coaching_rule": production.get("primary_coaching_rule") or "",
                "cluster_interpretations": production.get("cluster_interpretations") or {},
                "llm_error": s.get("llm_error_production") or s.get("llm_error_original"),
            },
            # Back-compat flat fields = next-day OS prompt
            "profile_version": current.get("profile_version"),
            "summary": current.get("summary") or "",
            "primary_coaching_rule": current.get("primary_coaching_rule") or "",
            "cluster_interpretations": current.get("cluster_interpretations") or {},
...
def main() -> None:
    ...
    payload = {
        "title": "Next-day overall_score clustering — cross-user impact analysis",
        "experiment": "next_day_overall_score",
        "alignment": "features_date_D__overall_score_date_D_plus_1",
        "notes": [
            "Same scenarios as same-day experiments, but overall_score is shifted from D+1.",
            "Non-OS features stay on feature_date D; rows without next-day OS are dropped.",
            "full_wearable = activity/sleep present; self_report_only = check-in + overall.",
            "Category completeness scores (not per-feature miss flags).",
            "behavioral_profiles_by_user = LLM interpretations: next-day OS system prompt "
            "(primary) + production same-day wording A/B.",
        ],
        "users_included": [
            {
                "user_id": u["user_id"],
        ...
        "metric_definitions": {
            ...
            "os_spread": "max(mean next-day OS) − min(mean next-day OS) across the 3 clusters.",
            "total_adjacent_gap": "gap(c0→c1) + gap(c1→c2) on mean next-day overall_score.",
    ...
    # Markdown report
    lines: list[str] = []
    lines.append("# Next-day overall_score clustering — cross-user impact analysis")
    lines.append("")
    lines.append(
        f"Same clustering scenarios as the same-day experiment, but **overall_score is from "
        f"D+1** while other features stay on date D. **{len(users)} users**; cohorts: "
        f"**{len(full_users)} full wearable**, **{len(self_users)} self-report only**."
    )
    lines.append("")
    lines.append("## Cohort")
    lines.append("")
    ...
    lines.append(
        ...
        "`cluster_0` = hardest/lowest **next-morning** overall_score tier; `cluster_2` = strongest. "
        "Each scenario shows **Next-day OS prompt** (experiment-specific system prompt) and "
        "**Production prompt** (same-day wording + next-day feature_timing)."
    )
    lines.append("")
    for entry in behavioral_profiles_by_user:
        ...
        for scenario_name in scenario_names:
            ...
            production = prof.get("original") or {}
            if production.get("summary") or production.get("llm_error"):
                _render_profile_block(lines, "Production prompt", production)
    ...
    lines.append("")
    lines.append(
        f"**Bottom line across these {len(users)} users:** with **next-day overall_score** "
        "alignment, clustering on **next-day OS alone** still tends to give the strongest "
        "OS-tier separation. Compare to the same-day experiment under `experiments/`."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append(
        ...
        "Regenerate with `python experiments/next_day_overall_score/_build_cross_user_analysis.py`."
    )
